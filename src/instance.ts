...
export const createCacheInstance = () => {
  const instanceCache: Cache<Tuple<unknown>>[] = []
  ...
  function query<Keys extends Tuple<unknown>, Fn extends (...keys: Keys) => Promise<unknown>>(
    ...
    for (const entry of instanceCache) {
      // Find a match
        ...
        fn(...keys)
          ...
          .then(() => {
            if (config.lifespan && config.lifespan > 0) {
              setTimeout(() => {
                const index = instanceCache.indexOf(entry)
                if (index !== -1) instanceCache.splice(index, 1)
              }, config.lifespan)
            }
          })
  ...
  const peek = <Keys extends Tuple<unknown>>(keys: Keys) =>
    instanceCache.find((entry) => shallowEqualArrays(keys, entry.keys, entry.equal))?.response

  const clear = <Keys extends Tuple<unknown>>(keys?: Keys) => {
    if (keys === undefined || keys.length === 0) instanceCache.splice(0, instanceCache.length)
    else {
      const entry = instanceCache.find((entry) => shallowEqualArrays(keys, entry.keys, entry.equal))
      if (entry) {
        const index = instanceCache.indexOf(entry)
        if (index !== -1) instanceCache.splice(index, 1)
      }
  ...
  return { suspend, clear, preload, peek }
}
